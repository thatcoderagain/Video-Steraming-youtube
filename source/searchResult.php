<?php
	...
	{
		$_GET['search_input'] = $_GET['search_input'];

		require_once 'init.searchResult.inc.php';
...
?>
						...
						<div class="col-xs-12 col-xs-offset-0 | col-sm-12 | col-md-12 | col-lg-12 text-left" id="form-box">
							...
							<div class="col-xs-12 col-xs-offset-0 | col-sm-10 col-sm-offset-1 | col-md-10 col-md-offset-1 | col-lg-10 col-lg-offset-1 text-left">
								<h4 class="help-block"><strong>Channels</strong></h4>
							</div>
...
<?php
		...
		{			
			...
			# QUERY
			$query = "SELECT `id`, `name`, `slug`, `image_filename`, `description`, `created_at` FROM `channel` WHERE `channel`. `slug` = '" . mysqli_real_escape_string($dbconnection, $slug) . "'";
			...
			$result = mysqli_query($dbconnection, $query);
			$subscriber_num = mysqli_num_rows($result);
...
?>
								...
								<div class="container-fluid">
									...
									<!-- Video div -->
									<div class="col-xs-4 col-xs-offset-0 | col-sm-2 col-sm-offset-0 | col-md-1 | col-lg-1">
										<div class="">
											<a href="<?php 
														echo htmlentities('./channel.php?slug='.$slug);
													?>">
												<img width="96px" height="auto" style="margin-top: 12px;"
													src="<?php 
														echo ($image_filename != NULL)? htmlentities($channelPath.$image_filename): htmlentities($channelPath.$defaultChannel); 
														?>">
											</a>
										</div>
									</div>
									...
									<!-- Video detail div -->
									<div class="col-xs-8 col-xs-offset-0 | col-sm-7 col-sm-offset-1 | col-md-10 col-md-offset-1 | col-lg-10 col-lg-offset-1">
										...
										<div class="row">
											...
											<div class="col-xs-12 | col-sm-12 | col-md-12 | col-lg-12">
												<h6 class="help-block">
													<?php 
														echo htmlentities($subscriber_num . ' subscribers') . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . htmlentities(date('jS M,Y . g:ia', strtotime($created_at))); 
													?>
												</h6>
											</div>
...
?>
							...
							<nav aria-label="Page navigation centered">
								<ul class="pagination">
									<li>
										<a href="<?php echo htmlentities('./searchResult.php?search_input='.$_GET['search_input'].'&page='.($page-1)); ?>" aria-label="Previous">
										<span aria-hidden="true">&laquo;</span>
										</a>
									</li>
									<li <?php if($page == 1 || $page == 2) echo 'class="hidden"'; ?>><a href="<?php echo htmlentities('./searchResult.php?search_input='.$_GET['search_input'].'&page='.($page-2)); ?>"><?php echo $page-2; ?></a></li>
									<li <?php if($page == 1) echo 'class="hidden"'; ?>><a href="<?php echo htmlentities('./searchResult.php?search_input='.$_GET['search_input'].'&page='.($page-1)); ?>"><?php echo $page-1; ?></a></li>
									<li class="active"><a href="<?php echo htmlentities('./searchResult.php?search_input='.$_GET['search_input'].'&page='.($page)); ?>"><?php echo $page; ?></a></li>
									<li <?php if($next == 0) echo 'class="hidden"'; ?>><a href="<?php echo htmlentities('./searchResult.php?search_input='.$_GET['search_input'].'&page='.($page+1)); ?>"><?php echo $page+1; ?></a></li>
									<li <?php if($next == 0) echo 'class="hidden"'; ?>><a href="<?php echo htmlentities('./searchResult.php?search_input='.$_GET['search_input'].'&page='.($page+2)); ?>"><?php echo $page+2; ?></a></li>
									<li <?php if($next == 0) echo 'class="hidden"'; ?>>
										<a href="<?php echo htmlentities('./searchResult.php?search_input='.$_GET['search_input'].'&page='.($page+1)); ?>" aria-label="Next">
